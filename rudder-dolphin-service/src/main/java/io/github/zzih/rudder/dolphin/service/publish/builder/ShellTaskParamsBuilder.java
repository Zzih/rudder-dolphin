...
/** Rename {@code content} → {@code rawScript}, then convert into DS {@link ShellParameters}. */
@Component
...
public class ShellTaskParamsBuilder implements TaskParamsBuilder {
    ...
    public boolean supports(TaskType type) {
        return type == TaskType.SHELL;
    }

    @Override
    public ShellParameters build(Map<String, Object> source, TaskType type) {
        Map<String, Object> ds = new LinkedHashMap<>(source);
        Object content = ds.remove("content");
        if (content != null) {
            ds.put("rawScript", content);
        }
        return objectMapper.convertValue(ds, ShellParameters.class);
    }
}
