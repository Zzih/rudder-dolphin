...
public class PythonTaskParamsBuilder implements TaskParamsBuilder {
    ...
    public boolean supports(TaskType type) {
        return type == TaskType.PYTHON;
    }

    @Override
    public PythonParameters build(Map<String, Object> source, TaskType type) {
        Map<String, Object> ds = new LinkedHashMap<>(source);
        Object content = ds.remove("content");
        if (content != null) {
            ds.put("rawScript", content);
        }
        return objectMapper.convertValue(ds, PythonParameters.class);
    }
}
