...
/**
 * rudder SqlTaskParams/FlinkJarTaskParams → DS FlinkParameters
 * <p>
 * Handles: FLINK_SQL (subType=FLINK_SQL), FLINK_JAR (subType=FLINK_JAR)
 */
@Component
public class FlinkTaskParamsBuilder implements TaskParamsBuilder {

    private static final int DEFAULT_SLOT = 1;
    private static final int DEFAULT_PARALLELISM = 1;
    private static final int DEFAULT_TASK_MANAGER = 2;
    private static final String DEFAULT_JOB_MANAGER_MEMORY = "1024m";
    private static final String DEFAULT_TASK_MANAGER_MEMORY = "2048m";

    @Override
    public Map<String, Object> build(Map<String, Object> taskParams, String subType) {
        return "FLINK_JAR".equals(subType) ? buildJar(taskParams) : buildSql(taskParams);
    }
    ...
    private Map<String, Object> buildSql(Map<String, Object> taskParams) {
        ...
        if (sql == null || sql.isBlank()) {
            throw new BizException(PublishErrorCode.INVALID_TASK_DEFINITION,
                    "Flink SQL task 'sql' is required");
        }

        Map<String, Object> ds = new LinkedHashMap<>();
        ds.put("programType", "SQL");
        ds.put("rawScript", sql);
        ds.put("initScript", taskParams.getOrDefault("initScript", ""));
        ds.put("mainClass", "");
        ds.put("mainJar", null);
        ds.put("mainArgs", "");
    ...
    private Map<String, Object> buildJar(Map<String, Object> taskParams) {
        ...
        if (mainClass == null || mainClass.isBlank()) {
            throw new BizException(PublishErrorCode.INVALID_TASK_DEFINITION,
                    "Flink JAR task 'mainClass' is required");
        }
        if (jarPath == null || jarPath.isBlank()) {
            throw new BizException(PublishErrorCode.INVALID_TASK_DEFINITION,
                    "Flink JAR task 'jarPath' is required");
        }
    ...
    private void applyResourceDefaults(Map<String, Object> ds, Map<String, Object> taskParams) {
        ...
        ds.put("deployMode", taskParams.getOrDefault("deployMode", "cluster"));
        ds.put("flinkVersion", taskParams.getOrDefault("flinkVersion", ">=1.12"));
        ds.put("slot", getResourceInt(resource, "slot", DEFAULT_SLOT));
        ds.put("parallelism", getResourceInt(resource, "parallelism", DEFAULT_PARALLELISM));
        ds.put("taskManager", getResourceInt(resource, "taskManager", DEFAULT_TASK_MANAGER));
        ds.put("jobManagerMemory", getResourceMemory(resource, "jobManagerMemory", DEFAULT_JOB_MANAGER_MEMORY));
        ds.put("taskManagerMemory", getResourceMemory(resource, "taskManagerMemory", DEFAULT_TASK_MANAGER_MEMORY));
