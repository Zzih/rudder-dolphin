...
package io.github.zzih.arion.dolphin.client;
...
public class ArionClient {
    ...
    public ArionClient(String baseUrl, String token) {
    ...
    private void post(String path, Object request) {
        ...
        } catch (ArionException e) {
            throw e;
        } catch (Exception e) {
            log.error("Arion API call failed: {}", url, e);
            throw new ArionException("Arion API call failed: " + url, e);
        }
    }
    ...
    private void checkResult(String response, String url) {
        try {
            ...
            if (code != 0) {
                ...
                throw new ArionException(code, message);
            }
        } catch (ArionException e) {
            throw e;
        } catch (Exception e) {
            log.error("Failed to parse Arion response: {}", url, e);
            throw new ArionException("Failed to parse Arion response", e);
        }
    }
}
